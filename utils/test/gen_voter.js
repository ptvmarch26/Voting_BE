...
//  * (Tôi dùng phiên bản tốt hơn từ script gen_voter.js trước đó của bạn)
//  */
...
//  * (Đã refactor để nhận eddsa build sẵn, giúp chạy nhanh hơn trong vòng lặp)
//  */
...
// async function main() {
//   // === Cấu hình ===
//   const NUM_VOTERS_TO_GENERATE = 100; // Thay đổi số lượng bạn muốn
...
//     __dirname,
//     `voter_data_${NUM_VOTERS_TO_GENERATE}.json`
//   );
...
//     // 1. Tạo key secp256k1 (giống frontend)
//     // Chúng ta chỉ cần pk_secp cho vào CSDL
//     const { pkHex: pk_secp } = generateSecpKeys();

//     // 2. Tạo key BabyJubJub (giống frontend)
//     const { pk: pk_bjj } = generateBabyJubJubKeys(eddsa);

//     // 3. Tính toán hashed_key (giống logic backend)
...
//     // 4. Tạo đối tượng data để lưu vào CSDL
//     const voterEntry = {
//       hashed_key: hashedKey,
//       election_id: ELECTION_ID,
//       is_valid: true, // Mặc định là true khi tạo
//       pk_secp: pk_secp,
//     };

//     voterDataList.push(voterEntry);
...
//   fs.writeFileSync(
//     outputJsonPath,
//     JSON.stringify(voterDataList, null, 2), // (null, 2) để format file JSON cho đẹp
//     "utf8"
//   );

//   console.log(
//     `\n💾 Success! Wrote ${voterDataList.length} voters to ${path.basename(
//       outputJsonPath
//     )}`
//   );
//   console.log("🎉 Done!");
...
async function main() {
  // === Cấu hình ===
  const NUM_VOTERS_TO_GENERATE = 1000; // Thay đổi số lượng bạn muốn
  const ELECTION_ID = "ELC2024"; // ID cuộc bầu cử
  // =================
  ...
  const elecBytes = Array.from(ELECTION_ID).map((c) =>
  ...
  console.log(
    `💾 [File 2] Wrote ${secretDataList.length} voter secrets to ${path.basename(
      outputScriptJsonPath
    )}`
  );
